...
function Home() {
  ...
  return RestaurantList.length === 0 ? (
    ...
    <div>
      <div className="flex w-2/3 items-center justify-around mb-10">
        <div className="">
          <input
            type="text"
            className="border rounded-md border-solid border-black mr-8 focus:ring-slate-500"
            onChange={(e) => setText(e.target.value)}
            value={text}
          ></input>
          {/* react will rerender as the value of text changes if write one letter in the input it will rerender the component */}
          <button className="border-2 w-40 py-1 rounded-lg shadow-md bg-slate-50 hover:shadow-xl" onClick={handleFilterSearch}>
            search
          </button>
    ...
    it will trigger the reconcilation proccess body component will be rerendered and rerender with updated value using diff */}
        ...
        <button className="border-2 w-64 py-3 rounded-lg shadow-lg bg-slate-5 hover:shadow-xl" onClick={handleFilter}>
